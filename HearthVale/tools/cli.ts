...
Data pipeline:
  ...
  rathena-audit <path/to/rathena/repo> [--max-level <n>] [--top <n>] [--limit <n>] [--out <path>]  Compatibility scan against a local rAthena checkout
  ...
  verify-resources    Gatherable resource-node and item-source check
  verify-economy      Shop stock and crafting recipe check
  verify-quests       Quest / warp unlock check
  verify-jobs         Job class catalog check
  verify-skills       Skill catalog check (incl. job startingSkills refs)
  verify-audio        Audio manifest check
  verify-assets       Asset manifest vs map assetKeys
  verify-biomes       Map biome palette coverage check
...
const scriptMap: Record<string, string> = {
  ...
  'verify-resources': 'scripts/verify-resources.ts',
  'verify-economy': 'scripts/verify-economy.ts',
  'verify-quests': 'scripts/verify-quests.ts',
  'verify-jobs': 'scripts/verify-jobs.ts',
  'verify-skills': 'scripts/verify-skills.ts',
  'verify-audio': 'scripts/verify-audio.ts',
  'verify-assets': 'scripts/verify-assets.ts',
  'verify-biomes': 'scripts/verify-biomes.ts',
  'rathena-audit': 'tools/rathena-audit.ts',
