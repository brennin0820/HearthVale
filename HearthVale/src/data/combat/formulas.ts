/** Original HearthVale combat formulas — RO-inspired structure, not copied literals. */
...
export function calcPhysicalDamage(
  ...
  return Math.max(1, Math.floor(raw * levelFactor));
}
...
export function rollHit(attacker: CombatStats, defender: CombatStats, rng = Math.random): boolean {
